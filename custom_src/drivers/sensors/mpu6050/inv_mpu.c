...
#if defined MOTION_DRIVER_TARGET_MSPM0G3507
...
uint8_t mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res){
            log_e("mpu_dmp_init设置所需要的传感器失败\n");
            return 1;
        }
		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置FIFO
		if(res){
            log_e("mpu_dmp_init设置FIFO失败\n");
            return 2; 
        }
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率
		if(res){
            log_e("mpu_dmp_init设置采样率失败\n");
            return 3; 
        } 
		res=dmp_load_motion_driver_firmware();		//加载dmp固件
		if(res){
            log_e("mpu_dmp_init加载dmp固件失败\n");
            return 4; 
        }
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向
		if(res){
            log_e("mpu_dmp_init设置陀螺仪方向失败\n");
            return 5; 
        }
		...
		if(res){
            log_e("mpu_dmp_init设置dmp功能失败\n");
            return 6; 
        }
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过200Hz)
		if(res){
            log_e("mpu_dmp_init设置DMP输出速率失败\n");
            return 7;
        }
		res=run_self_test();		//自检
		if(res){
            log_e("mpu_dmp_init自检失败\n");
            return 8;
        }  
		res=mpu_set_dmp_state(1);	//使能DMP
		if(res){
            log_e("mpu_dmp_init使能DMP失败\n");
            return 9;
        }
	}else {
        log_e("mpu_init初始化失败\n");
...
{
	float q0=1.0f,q1=0.0, q2=0.0f,q3=0.0f;
