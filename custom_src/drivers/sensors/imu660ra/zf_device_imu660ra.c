...
//-------------------------------------------------------------------------------------------------------------------
static imu660ra_state_enum imu660ra_expansion_get_acc(imu_interface_obj_struct *interface,
                                                      imu660ra_measurement_data_struct *measurement_data) {
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    do {
        if (interface->assert(NULL != measurement_data,    // 缓冲区指针 不能为空
        ...
        measurement_data->x = (int16_t)(((uint16_t)data[2] << 8 | data[1]));
        measurement_data->y = (int16_t)(((uint16_t)data[4] << 8 | data[3]));
        measurement_data->z = (int16_t)(((uint16_t)data[6] << 8 | data[5]));
    } while (0);
    return return_state;
}
...
// 备注信息     本接口为传感器框架接口 并不直接对用户开放
//-------------------------------------------------------------------------------------------------------------------
static imu660ra_state_enum imu660ra_expansion_get_gyro(imu_interface_obj_struct *interface,
                                                       imu660ra_measurement_data_struct *measurement_data) {
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    do {
        if (interface->assert(NULL != measurement_data,    // 缓冲区指针 不能为空
...
// 备注信息     本接口为传感器框架接口 并不直接对用户开放
//-------------------------------------------------------------------------------------------------------------------
static imu660ra_state_enum imu660ra_expansion_set_config(imu_interface_obj_struct *interface,
                                                         imu660ra_config_item_enum item) {
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    do {
        switch ((item >> IMU660RA_CONFIG_TYPE_OFFSET) & IMU660RA_CONFIG_TYPE_MASK) {
            case 0x00: {
                if (interface->assert(IMU660RA_ACC_OUTPUT_RATE_64_DIV <= (item & 0x0F) &&
            ...
            case 0x01: {
                if (interface->assert(
                        IMU660RA_ACC_RANGE_SGN_2G <= (item & 0x0F) && IMU660RA_ACC_RANGE_SGN_16G >= (item & 0x0F),
                        "item set error.",  // 输出预设报错信息
                        __FILE__, __LINE__)) {
            ...
            case 0x02: {
                if (interface->assert(IMU660RA_GYRO_OUTPUT_RATE_2_DIV <= (item & 0x0F) &&
            ...
            case 0x03: {
                if (interface->assert(IMU660RA_GYRO_RANGE_SGN_125DPS <= (item & 0x0F) &&
...
static imu660ra_state_enum imu660ra_expansion_init(imu_interface_obj_struct *interface) {
    ...
    do {
        ...
        interface->write_8bit_registers(IMU660RA_DEV_ADDR, IMU660RA_INIT_DATA, imu660ra_config_file,
        ...
        if (interface->assert(1 == data[1],              // 检查是否配置完成
        ...
        if (interface->assert(IMU660RA_ACC_OUTPUT_RATE_64_DIV <= IMU660RA_ACC_OUTPUT_RATE_DEFAULT &&
        ...
        interface->write_8bit_register(IMU660RA_DEV_ADDR, IMU660RA_ACC_CONF,
                                       0xA0 | (IMU660RA_ACC_OUTPUT_RATE_DEFAULT - 1));

        if (interface->assert(IMU660RA_GYRO_OUTPUT_RATE_2_DIV <= IMU660RA_GYRO_OUTPUT_RATE_DEFAULT &&
        ...
        interface->write_8bit_register(IMU660RA_DEV_ADDR, IMU660RA_GYR_CONF,
                                       0xA0 | (IMU660RA_GYRO_OUTPUT_RATE_DEFAULT - 1));
        ...
        if (interface->assert(IMU660RA_ACC_RANGE_SGN_2G <= IMU660RA_ACC_RANGE_DEFAULT &&
        ...
        if (interface->assert(IMU660RA_GYRO_RANGE_SGN_125DPS <= IMU660RA_GYRO_RANGE_DEFAULT &&
...
// 备注信息     根据量程进行物理量换算
//              加速度计量程 ±2  g   获取到的加速度计数据除以 16384  可以转化为带物理单位的数据 (g 代表重力加速度
//              物理学名词 一般情况下 g 取 9.8 m/s^2 为标准值) 加速度计量程 ±4  g   获取到的加速度计数据除以 8192
//              可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下 g 取 9.8 m/s^2 为标准值) 加速度计量程
//              ±8  g   获取到的加速度计数据除以 4096   可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词
//              一般情况下 g 取 9.8 m/s^2 为标准值) 加速度计量程 ±16 g   获取到的加速度计数据除以 2048
//              可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下 g 取 9.8 m/s^2 为标准值)
//-------------------------------------------------------------------------------------------------------------------
imu660ra_state_enum imu660ra_get_physical_acc(imu660ra_measurement_data_struct *measurement_data,
                                              imu660ra_acc_range_enum range,
                                              imu660ra_physical_data_struct *physical_data) {
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    do {
        if (imu_assert(NULL != measurement_data,    // 缓冲区指针 不能为空
                       "*measurement_data error.",  // 输出预设报错信息
                       __FILE__, __LINE__)) {
            return_state = IMU660RA_STATE_DATA_BUFFER_ERROR;
            break;
        }
        if (imu_assert(IMU660RA_ACC_RANGE_SGN_2G <= range && IMU660RA_ACC_RANGE_SGN_16G >= range,
                       "range error.",  // 输出预设报错信息
                       __FILE__, __LINE__)) {
            return_state = IMU660RA_STATE_SET_ACC_RANGE_ERROR;
            break;
        }
        if (imu_assert(NULL != physical_data,    // 缓冲区指针 不能为空
...
//-------------------------------------------------------------------------------------------------------------------
imu660ra_state_enum imu660ra_get_physical_gyro(imu660ra_measurement_data_struct *measurement_data,
                                               imu660ra_gyro_range_enum range,
                                               imu660ra_physical_data_struct *physical_data) {
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    do {
        if (imu_assert(NULL != measurement_data,    // 缓冲区指针 不能为空
        ...
        if (imu_assert(IMU660RA_GYRO_RANGE_SGN_125DPS <= range && IMU660RA_GYRO_RANGE_SGN_2000DPS >= range,
                       "range error.",  // 输出预设报错信息
                       __FILE__, __LINE__)) {
            return_state = IMU660RA_STATE_SET_GYRO_RANGE_ERROR;
            break;
...
imu660ra_state_enum imu660ra_init(void) {
    soft_iic_init(&imu_soft_iic_obj);
    imu660ra_state_enum return_state = IMU660RA_STATE_NO_ERROR;
    imu_delay_ms(20);  // 等待设备上电成功
    do {
        if (imu_assert(!imu_interface_init(),        // 如果没有异常 那么接口初始化返回为 0
        ...
        if (imu_assert(!imu660ra_expansion_ckeck_id(
                           &imu_interface_default_obj),  // IMU660RA 自检 如果没有异常 那么接口初始化返回为 0
