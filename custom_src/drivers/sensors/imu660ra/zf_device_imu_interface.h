...
typedef void (*imu_delay_ms_func)(uint32_t parameter);

typedef uint8_t (*imu_assert_func)(uint8_t pass, char *str, char *file, int line);

typedef void (*imu_write_8bit_register_func)(uint8_t addr, uint8_t reg, uint8_t data);
typedef void (*imu_write_8bit_registers_func)(uint8_t addr, uint8_t reg, const uint8_t *data, uint32_t len);
typedef uint8_t (*imu_read_8bit_register_func)(uint8_t addr, uint8_t reg);
typedef void (*imu_read_8bit_registers_func)(uint8_t addr, uint8_t reg, uint8_t *data, uint32_t len);


typedef struct  // IMU660RA 模块化解耦结构
{
    uint8_t interface_type;  // 必须要的接口类型
...
extern soft_iic_info_struct  imu_soft_iic_obj;
